...
import { generateChannelPlan, getWorkflow } from "@/lib/workflows.functions";
...
function MediaPlanPage() {
  ...
  const strategy = wf.data?.strategy as
      ...
      <Card className="max-w-2xl space-y-4 p-6">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <Button data-testid="generate-media-plan" disabled={g.isPending} onClick={() => g.mutate()}>
            {g.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <PieIcon className="mr-2 h-4 w-4" />}
        ...
        <p className="text-xs text-muted-foreground">
          {plan
            ...
            : strategy?.positioning
              ? "Not generated yet."
              : "Generate the strategy on the campaign page first."}
        </p>
      </Card>
