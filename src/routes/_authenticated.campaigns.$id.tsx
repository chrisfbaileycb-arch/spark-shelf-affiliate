...
function BriefStep({ id, data, refresh }: StepProps) {
  ...
  return (
    <Card className="max-w-3xl space-y-4 p-6">
      ...
      <Field label="Proof points (one per line — real facts only)" value={proof} onChange={setProof} rows={4} testId="brief-proof" />
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">
          {savedAt ? `Saved ${new Date(savedAt).toLocaleTimeString()}` : brief?.approved_at ? "Approved" : "Not saved yet"}
        </span>
      </div>
    </Card>
  );
...
function StrategyStep({ id, data, refresh }: StepProps) {
  ...
  return (
    <Card className="max-w-3xl space-y-4 p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {s?.generated_at ? `Generated ${new Date(s.generated_at).toLocaleString()} · ${s.model ?? ""}` : "Not generated yet"}
          </p>
        </div>
      ...
      <div className="flex gap-3">
        ...
        <Button data-testid="approve-strategy" disabled={m.isPending || !positioning.trim()} onClick={() => m.mutate(true)}>
          Approve &amp; continue
        </Button>
      </div>
    </Card>
...
function ContentStep({ id, data, refresh }: StepProps) {
  ...
  return (
    <div className="max-w-3xl space-y-4">
      ...
      <Card className="space-y-3 p-6">
        ...
        <p className="text-xs text-muted-foreground">
          Render images and video in Studio, then link the kit here. Video state is read from the
          kit itself — nothing shows as ready without a playable file.
        </p>
