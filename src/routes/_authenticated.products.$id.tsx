...
function ProductDetail() {
  ...
  const linkMut = useMutation({
    mutationFn: () => csl({ data: { product_id: id, affiliate_program_id: programId === "none" ? null : programId } }),
    onSuccess: () => {
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
        <div className="space-y-6">
          <header>
            ...
            {p.price ? <p className="mt-2 text-2xl font-semibold text-primary">{p.price} {p.currency}</p> : null}
          </header>
          ...
          <div className="flex gap-2">
            <a href={p.source_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
        ...
        <aside className="space-y-4">
          <Card className="space-y-4 bg-gradient-brand p-6 text-primary-foreground shadow-pop">
            ...
            >
              {generateMut.isPending ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Generating (~30s)…</>) : (<><Sparkles className="mr-2 h-4 w-4" /> Generate video</>)}
            </Button>
          </Card>

          <Card className="space-y-3 p-6">
            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Affiliate link</p>
            {suggested && (
              <div className="rounded-lg bg-surface p-3 text-xs">
                Suggested network: <strong>{suggested.name}</strong>
                <a href={suggested.signupUrl} target="_blank" rel="noopener noreferrer" className="ml-1 text-primary hover:underline">sign up</a>
                <p className="mt-1 text-muted-foreground">{suggested.notes}</p>
              </div>
            )}
            <Select value={programId} onValueChange={setProgramId}>
              <SelectTrigger><SelectValue placeholder="Pick your affiliate ID" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">No affiliate ID (raw URL)</SelectItem>
                {programs.data?.map((pg) => <SelectItem key={pg.id} value={pg.id}>{pg.name}</SelectItem>)}
              </SelectContent>
            </Select>
            <Button variant="secondary" onClick={() => linkMut.mutate()} disabled={linkMut.isPending} className="w-full">
            ...
            <Link to="/affiliate-programs" className="block text-center text-xs text-muted-foreground hover:text-foreground">Manage your affiliate IDs →</Link>
          </Card>
        </aside>
      </div>
    </div>
