...
function CheckoutReturn() {
  ...
  useEffect(() => {
    ...
    sync({ data: { sessionId: session_id, environment: getStripeEnvironment() } })
      .then((r) => { if ("ok" in r && r.ok) { setTier(r.tier); setState("ok"); } else setState("error"); })
  ...
  return (
    <div className="grid min-h-screen place-items-center p-6">
      <Card className="max-w-md p-8 text-center">
        {state === "loading" && <><Loader2 className="mx-auto h-8 w-8 animate-spin text-primary" /><p className="mt-4">Confirming your subscription…</p></>}
        {state === "ok" && (
          <>
            <CheckCircle2 className="mx-auto h-12 w-12 text-primary" />
            <h1 className="mt-3 font-display text-2xl">You're on {tier}!</h1>
            <p className="mt-2 text-sm text-muted-foreground">Your plan is active. Let's make videos.</p>
        ...
        {state === "error" && (
          <>
            ...
            <p className="mt-2 text-sm text-muted-foreground">We couldn't confirm the session. Check your billing page.</p>
            <Link to="/billing"><Button className="mt-6">View billing</Button></Link>
          </>
        )}
      </Card>
    </div>
