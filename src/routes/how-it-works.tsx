...
export const Route = createFileRoute("/how-it-works")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "See the 4-step pipeline: paste a product URL, AI ingests it, generates a 15s influencer video with voiceover, and hands you a tracked affiliate link." },
      { property: "og:title", content: "How ReelRipper works" },
      { property: "og:description", content: "From product URL to postable affiliate video in under a minute." },
  ...
  return (
    <div className="min-h-screen bg-background bg-grain">
      ...
      <section className="mx-auto max-w-4xl px-6 py-16 md:py-24">
        ...
        <p className="mt-4 text-lg text-muted-foreground">Four steps between a product URL and a video ready to post.</p>
        <div className="mt-12 space-y-8">
          {STEPS.map((s, i) => (
            <div key={i} className="flex gap-6 rounded-2xl border border-border bg-card p-6 shadow-pop">
              <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary"><s.i className="h-6 w-6" /></div>
              <div>
        ...
        <div className="mt-16 rounded-2xl border border-border bg-surface p-8 text-center">
          ...
          <Link to="/auth" className="mt-6 inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-pop hover:opacity-95">
