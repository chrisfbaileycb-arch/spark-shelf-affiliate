  ...
  return (
    <div className="space-y-6">
      <header className="flex items-end justify-between">
        ...
        <Button onClick={() => navigate({ to: "/products/new" })}><Plus className="mr-2 h-4 w-4" /> New</Button>
      </header>
      ...
      {q.isLoading ? (
              ...
              <Link to="/products/$id" params={{ id: p.id }} key={p.id}>
                <Card className="flex gap-4 p-4 transition hover:-translate-y-0.5 hover:shadow-pop">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {p.price ? <p className="mt-2 text-sm font-semibold text-primary">{p.price} {p.currency}</p> : null}
                  </div>
                </Card>
              </Link>
            );
        ...
        <Card className="border-dashed p-12 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">Paste any product URL — Amazon, TikTok Shop, AliExpress, Etsy, Shopify store — and we'll handle the rest.</p>
          <Button className="mt-6" onClick={() => navigate({ to: "/products/new" })}><Plus className="mr-2 h-4 w-4" /> Ingest first product</Button>
        </Card>
      )}
    </div>
  );
