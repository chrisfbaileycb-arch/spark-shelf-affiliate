...
export default defineTool({
  ...
  description: "Generate a 15-second influencer script, caption, and hashtags for a product without rendering a video.",
  inputSchema: {
  ...
  handler: async ({ product_id, persona_id }, ctx) => {
    ...
    let persona: { name: string; bio: string | null; vibe: string | null; voice_tone: string | null; catchphrases: unknown; speech_quirks: string | null } | null = null;
    if (persona_id) {
