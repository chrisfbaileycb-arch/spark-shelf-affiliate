...
export default defineTool({
  ...
  description:
    "Save an affiliate program with its tracking ID and link template for later use when building short links.",
  inputSchema: {
    ...
    link_template: z
      ...
      .min(4)
      .max(1000)
      .describe("URL template. Use {url} for the product URL and {tracking_id} for your tag."),
  ...
  handler: async ({ name, network, tracking_id, link_template, notes }, ctx) => {
    if (!ctx.isAuthenticated())
      throw new ToolError("You must be signed in to create an affiliate program.");
